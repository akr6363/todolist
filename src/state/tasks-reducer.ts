...
export const tasksReducer = (state: TasksType, action: TasksReducerActionType) => {
    switch (action.type) {
        ...
        case 'REMOVE-TODOLIST': {
            const {[action.todoListsId]: deletedKey, ...rest} = state
            return rest
        }
        default:
...
export const removeTaskAC = (taskId: string, todoListsId: string): removeTaskActionType => ({
    type: 'REMOVE-TASK', taskId, todoListsId
})
export const addTaskAC = (title: string, todoListsId: string): addTasksActionType => ({
    type: 'ADD-TASK', title, todoListsId
})
export const changeTaskStatusAC = (taskId: string, todoListsId: string, isDone: boolean): changeTaskStatusActionType => ({
    type: 'CHANGE-TASK-STATUS', taskId, todoListsId, isDone
})
export const changeTaskTitleAC = (taskId: string, todoListsId: string, title: string): changeTaskTitleActionType => ({
